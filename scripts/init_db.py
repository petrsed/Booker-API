...
def get_files():
    ...
    if not os.path.exists(db_scripts_path):
        logging.error("There is no 'db' folder with sql scripts in the project folder")
        return []
    ...
    if len(files) != 0:
        for file in files:
            try:
                ...
                logging.info(f"Execute - {filename}.")
        ...
        logging.error("There are no 'sql' scripts in the db folder")
